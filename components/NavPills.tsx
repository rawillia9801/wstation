...
export default function NavPills() {
  ...
  async function sendAlarmTest() {
    const res = await fetch('/api/send-alert-report', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        type: alarmTest,
        message: 'Manual trigger test from Smart Weather Alarm Center.'
      })
    })
    const data = await res.json()
    alert(JSON.stringify(data))
      ...
      {showSettings && (
        <div className="absolute right-4 top-24 w-[520px] max-w-[90vw] rounded-3xl border border-cyan-300/30 bg-slate-950/95 backdrop-blur-xl p-5 z-[100] text-white">
          ...
          <div className="mb-4">
            ...
            <textarea value={emails} onChange={e => setEmails(e.target.value)} className="w-full rounded-xl bg-black/50 border border-cyan-400/20 px-3 py-2 text-sm h-20" />
          </div>
          <div className="mb-4">
            <div className="text-cyan-200 text-sm mb-2">SMS Alert Recipients</div>
            <textarea value={phones} onChange={e => setPhones(e.target.value)} className="w-full rounded-xl bg-black/50 border border-cyan-400/20 px-3 py-2 text-sm h-20" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button onClick={saveSettings} className="rounded-xl bg-cyan-500/20 border border-cyan-300/30 py-2 text-sm">Save Preferences</button>
            <button onClick={sendTest} className="rounded-xl bg-green-500/20 border border-green-300/30 py-2 text-sm">Send Test Alert</button>
          </div>
        </div>
      )}
      ...
      {showAlarms && (
        <div className="absolute right-4 top-24 w-[760px] max-w-[94vw] rounded-3xl border border-red-300/20 bg-slate-950/95 backdrop-blur-xl p-6 z-[100] text-white max-h-[78vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3 mt-5">
            <input value={alarmTest} onChange={e => setAlarmTest(e.target.value)} className="rounded-xl bg-black/50 border border-red-300/30 px-3 py-2 text-sm" />
            <button onClick={sendAlarmTest} className="rounded-xl bg-red-500/20 border border-red-300/30 py-2 text-sm">Send Trigger Test</button>
          </div>
        </div>
      )}
    </>
